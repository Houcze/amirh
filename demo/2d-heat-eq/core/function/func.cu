...
int laplace(double* phi, double* result, int N1, int N2)
{
    ...
    parti_diff_k<<<dim3(std::ceil(double(N1 * N2) / threadsperblock), 1, 1), dim3(threadsperblock, 1, 1)>>>(phi, result, make_int2(N1, N2), make_int2(1, 0));
    parti_diff_k<<<dim3(std::ceil(double(N1 * N2) / threadsperblock), 1, 1), dim3(threadsperblock, 1, 1)>>>(phi, result, make_int2(N1, N2), make_int2(-1, 0));
    parti_diff_k<<<dim3(std::ceil(double(N1 * N2) / threadsperblock), 1, 1), dim3(threadsperblock, 1, 1)>>>(phi, result, make_int2(N1, N2), make_int2(0, 1));
    parti_diff_k<<<dim3(std::ceil(double(N1 * N2) / threadsperblock), 1, 1), dim3(threadsperblock, 1, 1)>>>(phi, result, make_int2(N1, N2), make_int2(0, -1));
